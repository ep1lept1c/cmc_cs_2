...
double integral(double (* f)(double), double a, double b, double eps2){       // функция вычисления интеграла с помощью метода трапеций
    ...
    double step = (b - a)/(2*n);                    // шаг разбиения
    double In = (f(a) + f(b))/2;               // значение определенного интеграла с n разбиениями
    double I2n = In;                          // значение определенного интеграла с 2n разбиениями
    for(int i = 1; i < 2*n; i++){
        I2n += f(a + step*i);                // складываем основания трапеций
        if (i % 2 == 0){                     // в In попадает лишь половина оснований, т.е. мы не пересчитываем в I2n старые значения
            In += f(a + step*i);            //  в I2n новые отрезки для разбиения становятся в середину между старыми
        }
    }
    In *= step*2;                                       // умножаем на половину высоты (т.к Sтр = (a+b)*(h/2))
    I2n *= step;                                      // шаг для I2n был в 2 раза меньше
    if (fabs(In - I2n) >= eps2){
        return 0;
    }
    return I2n;
